...
FILES = [
    ...
    "detector/ozon_m105_profile.py",
    "detector/ozon_serializer_mix.py",
...
ensure_dir(f"{cfg.BOT_DIR}/detector/ozon_v1")
for rel in FILES:
...
print("Set OZON_V1_ROLLOUT=full ...")
print(run(
    f"mkdir -p /etc/systemd/system/pdfbot.service.d && "
    f"printf '[Service]\\nEnvironment=OZON_V1_ROLLOUT=full\\n' > "
    f"/etc/systemd/system/pdfbot.service.d/ozon-v1.conf && systemctl daemon-reload"
))
...
sftp.close()
ssh.close()
print("Done — Ozon v1 full mode active")
