"""Deploy T-Bank F2 .notdef integrity (v6.3.0) to Aeza."""
...
FILES = [
    "detector/tbank_notdef_integrity.py",
...
fake = Path(
    r"C:\Users\fanis\OneDrive\Desktop\ЗАПАСКА 13.07.26\чеки_30_из_30\sbp30_18.pdf"
).read_bytes()
with sftp.file("/tmp/sbp30_18.pdf", "wb") as f:
    f.write(fake)
...
print(run(
    ...
    "r=analyze(Path('/tmp/sbp30_18.pdf').read_bytes()); "
    "print(VALIDATOR_VERSION); print(r['verdict'], r['score']); "
    "print([x for x in r.get('flags') or [] if 'NOTDEF' in x or 'AMOUNT' in x "
    "or 'USED_CID_MISSING' in x])\""
))
sftp.close()
ssh.close()
print("DEPLOYED")
