"""Deploy Ozon: drop OZON_SBP_ID_CROSS_BANK_TAIL (G101 genuines)."""
...
SMOKE = Path(r"C:\Users\fanis\OneDrive\Desktop\чеки\озон чекии\sbp.pdf")
...
if SMOKE.is_file():
    with sftp.file("/tmp/ozon_g101_smoke.pdf", "wb") as f:
        f.write(SMOKE.read_bytes())
    print(run(
        "cd /root/pdf-checker-bot && PYTHONPATH=. venv/bin/python -c "
        "\"from pathlib import Path; from detector.ozon import analyze; "
        "r=analyze(Path('/tmp/ozon_g101_smoke.pdf').read_bytes()); "
