"""Alfa v1.0 expert report for @kronlead (spec §16)."""
...
def _lines(flag: AlfaFlag, idx: int) -> list[str]:
...
def build_expert_report(pipeline: PipelineResult, verdict: str) -> dict:
    ...
    if verdict == "ФЕЙК":
        ...
        body.append("")
        body.append(
            "Полный Alfa pipeline завершён. Доказанных структурных, шрифтовых, "
            "визуальных и идентификационных противоречий не найдено."
        )
        checks = ", ".join(pipeline.completed_checks) or (
            "revisions/xref, objects, streams, active content, content AST, "
            "fonts/CMap/W/glyph, семантика, identifiers, parser parity"
        )
