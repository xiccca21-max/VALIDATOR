"""Sber v1 rollout: shadow mode + staged feature flag (spec §17)."""
...
def rollout_mode() -> str:
    ...
    return (os.environ.get("SBER_V1_ROLLOUT") or _DEFAULT_MODE).strip().lower()
...
def apply_rollout(v1_result: dict, pdf_bytes: bytes, file_hash: str) -> dict:
    ...
    if mode == "shadow":
        ...
        from ..sber_legacy import analyze as analyze_legacy



        legacy = analyze_legacy(pdf_bytes, file_hash)

        details["sber_v1_shadow"]["legacy_verdict"] = legacy.get("verdict")

        details["sber_v1_shadow"]["match"] = (

            legacy.get("verdict") == v1_result.get("verdict")

        )

        if not details["sber_v1_shadow"]["match"]:

            log.info(

                "sber_v1 shadow mismatch hash=%s v1=%s legacy=%s",
        ...
        legacy_details["sber_v1_shadow"] = details["sber_v1_shadow"]
